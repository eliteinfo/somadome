...
{
    $strFile = "scat_csv.csv";
    if($_FILES['txtfile']['name']!='')
    {
		move_uploaded_file($_FILES['txtfile']['tmp_name'],"subcategory_csv/scat_csv.csv");
		$arrData = readCSV("subcategory_csv/scat_csv.csv");
		foreach($arrData as $intKey=>$strValue)
		{
		   ...
		   if($intKey!=0)
		   {
				...
				$objData->setFieldValues("sname",$strValue[1]);
				$objData->insert();
		   }
		} 
    }else
    ...
    <div class="row-fluid">
        <div class="span12">
            ...
            <ul class="breadcrumb">
                ...
                <li>
                    <a href="list_subcategory.php">Subcategory</a>
                    <span class="icon-angle-right"></span>
                </li>
                <li>Add Subcategory</li>
                <div class="btn-group" style="margin-top:-7px; float:right">
                    <a href="list_subcategory.php"><button id="sample_editable_1_new" class="btn green">
                        List Subcategory
                        </button></a>
                </div>
            </ul>
