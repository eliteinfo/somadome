...
$arrUpcomming = $objData->getAll("Select suub.*,u.Fname,u.Lname,su.Id as d_id,su.Cus_id,su.unitname from soma_user_unit_booking suub, soma_users u, soma_units su where suub.Uid = u.Uid AND suub.dom_id = su.Id  AND suub.book_status = '1' order by suub.Id desc");

/*echo"Select suub.*,u.Fname,u.Lname,su.Id,su.unitname from soma_user_unit_booking suub, soma_users u, soma_units su where suub.Uid = u.Uid AND suub.dom_id = su.Id  AND suub.book_status = '1' order by suub.Id desc";*/

if(isset($_REQUEST['search']) && $_REQUEST['search']!='')
{
	//echo"<pre>";print_r($_POST);exit;
	if($_POST['domsFilter'] != ''){
		$arrUpcomming = $objData->getAll("Select suub.*,u.Fname,u.Lname,su.Id as d_id,su.Cus_id,su.unitname from soma_user_unit_booking suub, soma_users u , soma_units su where suub.Uid = u.Uid AND suub.dom_id = su.Id  AND suub.book_status = '1' AND suub.dom_id = '".$_POST['domsFilter']."' order by suub.Id desc");
	}else{
		$arrUpcomming = $objData->getAll("Select suub.*,u.Fname,u.Lname,su.Id as d_id,su.Cus_id,su.unitname from soma_user_unit_booking suub, soma_users u, soma_units su where suub.Uid = u.Uid AND suub.dom_id = su.Id  AND suub.book_status = '1' order by suub.Id desc");
	}
            ...
            <div class="span12">
                ...
                <div class="portlet box green">
                    <div class="portlet-title">
                        <div class="caption"><i class="icon-globe"></i>List Upcomming</div>
                    </div>
                    ...
                    <form  method="post" id="form_sample_2" name="form_sample_3" class="form-horizontal" enctype="multipart/form-data">
                        ...
                        <?php if(!empty($arrUpcomming)) { ?>
                            ...
                            <tbody>
                             <?php  for($i=0;$i<count($arrUpcomming);$i++)
							 { 
                                 ...
                                 <tr class="odd gradeX">
                                	...
                                	<td><?php echo ($i+1);?></td>
                                    <td><?php echo $arrUpcomming[$i]['Fname'] ?></td>
                                    <td><?php echo $arrUpcomming[$i]['from_date'].' '.$arrUpcomming[$i]['from_time'] ?></td>
									<td><?php echo $arrUpcomming[$i]['to_date'].' '.$arrUpcomming[$i]['to_time'] ?></td>
                                    <td><?php echo $arrUpcomming[$i]['unitname'];?></td>
                                    <?php $hostName = $objData->getAll("Select * from soma_customers where Cid = '".$arrUpcomming[$i]['Cus_id']."'");?>      
