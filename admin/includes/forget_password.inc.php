...
{
	$is_exist = "select * from soma_admin where email = '".$_POST['email']."'";
	$db_exist_email = $objData->getAll($is_exist);
	if(!empty($db_exist_email))
	{
		...
		$update_page="UPDATE soma_admin
					  SET 
					  	apass = '".md5($new_pass)."'
					  where 
					  	email = '".$_POST['email']."'";						
		
   	   $db_updatepage=$objData->getAll($update_page);	
		...
		$headers .= 'From: Somadome <' . $objData->INFO_MAIL . '>';
