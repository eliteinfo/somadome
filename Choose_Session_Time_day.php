...
<div class="top_head">
    <div class="wrapper">
        <div class="tpleft">
            ...
            <div class="tptxt">
                <h2><?php echo ucfirst($arrDom[0]['unitname']); ?></h2>
                <?php 
            ...
            <div class="tptxt tptxt2">
                <?php $Today=$_GET['dt'];if($Today==''){$Today = date('Y-m-d'); }?>
    ...
    if(strtolower($weekDay) == 'saturday' || strtolower($weekDay) == 'sunday'){ ?>
        ...
        if($SaMtime=='0'){ $SaMtime = '00'; }
        $EaMtime=$arrDom[0]['unit_weekendclose']%60;
        if($EaMtime=='0'){ $EaMtime = '00'; }
...
<div class="leftbar leftbar_new">  
	...
	<?php include('Choose_Session_Day_Leftbar.php');?>
            ...
            <div class="calendar_part">
                <div class="weekdays">
                    ...
                    if (!isset($_GET['dt'])) {
                        ...
                        $sdt = date("Y-m-d", strtotime($_GET['dt']));
                    }
                    for ($i = 0; $i < 1; $i++) { ?>
                        <?php $DateList = date("Y-m-d", strtotime($sdt . "+ " . $i . " days")); ?>
                        <div class="week_box day_full_box"><?php echo date("D d F", strtotime($DateList)); ?></div>
                    <?php } ?>
                </div>
                <?php
               ...
               <?php  //echo $sdt;?>
               <?php  $Days = date('l', strtotime($sdt));?>
               <?php if($Days == 'Saturday' || $Days == 'Sunday'){ ?>
			   ...
			   <?php }else{ ?>
                ...
                <div class="date_mainbox">
                    ...
                    <div class="days_box"><?php //echo $tm; ?><?php echo date('h:i A', strtotime($tm)); ?></div>
                    <?php
                    for ($k = 0; $k < 1; $k++) {
                        $DateList = date("Y-m-d", strtotime($sdt . "+" . $k . " days"));
                        if (!isset($_GET['Did'])) {
                            $arrBooking = $objData->getAll("SELECT * FROM  soma_user_unit_booking WHERE book_status!='2' and dom_id ='" . $arrDom[0]['Id'] . "' and from_date='" . $DateList . "' and from_time='" . $tm1 . "'");
							
							$arrCancel = $objData->getAll("SELECT * FROM  soma_user_unit_booking WHERE book_status!='1' and dom_id ='" . $arrDom[0]['Id'] . "' and from_date='" . $DateList . "' and from_time='" . $tm1 . "' and Uid = '".$_SESSION['Uid']."'");

                        } else {
                            $arrBooking = $objData->getAll("SELECT * FROM  soma_user_unit_booking WHERE book_status!='2' and dom_id='" . $_GET['Did'] . "' and from_date='" . $DateList . "' and from_time='" . $tm1 . "'");
							
							$arrCancel = $objData->getAll("SELECT * FROM  soma_user_unit_booking WHERE  book_status!='1' and dom_id='" . $_GET['Domid'] . "' and from_date='" . $DateList . "' and from_time='" . $tm1 . "' and Uid = '".$_SESSION['Uid']."'");
                        }
						
                        $divid = $arrDom[0]['Id'] . '_' . str_replace(":", "-", $tm1) . '_' . $DateList;
                        ?>
                        <?php $ttS1 = $DateList.' '.$tm1 ;?>
                        <?php 
						if( strtotime($ttS1) > strtotime('now') ) { ?>
						<div <?php if (count($arrBooking) == "0" && count($arrCancel) == "0") { ?> class="days_box available span_design_avail"
                         <?php } else if(count($arrBooking)>0) { ?>   <?php if($arrBooking[0]['Uid'] == $_SESSION['Uid']) {?> style="background-color: #0000FF;" class="days_box span_design_avail" <?php }else{ ?> style="background-color: #ccc;" class="days_box span_design_avail" <?php } ?>   <?php } ?>
                         <?php if (count($arrCancel) > "0" && count($arrBooking) == "0"){?>style="background-color: #FF0000;" class="days_box cancel span_design_avail"<?php } ?>
                         id="<?php echo $divid; ?>">
                        <span <?php if (count($arrBooking)== "0" && count($arrCancel) == "0") { ?> class="spntxt available_span span_design_avail" <?php } else{?> class="spntxt span_design_avail" <?php } ?> id="<?php echo $divid . '_spantxt'; ?>"><?php if (count($arrBooking) > "0") {echo "Booked";}else{ ?> <?php echo"Cancelled"; } ?></span>
                    </div>
					<?php }else{ ?>
						<div <?php if (count($arrBooking) == "0") { ?> class="days_box span_design_avail"
                         <?php } else if(count($arrBooking)>0) { ?>style="background-color: #00FF00;" class="days_box span_design_avail"<?php } ?>
                         <?php if (count($arrCancel) > "0" && count($arrBooking) == "0"){?>style="background-color: #FF0000;" class="days_box"<?php } ?>
                         id="<?php echo $divid; ?>">
                        <span <?php if (count($arrBooking)== "0") { ?> class="spntxt span_design_avail" <?php } else{?> class="spntxt span_design_avail" <?php } ?> id="<?php echo $divid . '_spantxt'; ?>"><?php if (count($arrBooking) > "0") {echo "Completed";}else{ ?> <div class="unavailable" style="background-color:#ccc;"> <span><?php echo"Unavailable";  ?></span></div><?php } ?>  </span>
                    </div>
					<?php } ?>
...
/*function booking(){
	...
	jQuery.ajax({
		   url: 'ajax.php',
		   data: {title: title,start_Date:start,end_date:end,dom_id:d_id,time:stime,CMD:"Event_Add"},
		   type: 'POST',
		   cache: true,
		   success: function (data)
		   {
			   ...
			   {
                   $('#'+value).css('background-color','#9559d6');
...
jQuery("#dom_select").change(function() {
  var url = '<?php echo $objData->SITEURL ?>Choose_Session_Time_day.php'+'?Cid='+'<?php echo $_GET['Cid']; ?>';
