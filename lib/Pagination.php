...
define('MAX_RECORD_PER_PAGE',10);									// To Show Records per page in Listing Page
define('MAX_PAGE_NUMBER_PER_SECTION',5);	
define('ADMIN_ID','1');
class Pagination
{
	/*******************************************************
	...
	*******************************************************/
	...
	{	
		...
		// Determine Style
		$style = ($this->_currentPage == $destinationPage && !$end) ? "btn" : $this->_styles["pageOff"];
			...
			&& $destinationPage >= 1)						// and destination page is not less than first page
		{
					...
					{
							if($this->_getURL($destinationPage)==""){
							$text = '<a href="'. $this->_getURL($destinationPage) .'" style="background-image: -moz-linear-gradient(center top , #C9C9C9, #C9C9C9);" >'. $text .'</a>'; // then make $text a link
							}else{
									$text = '<a href="'. $this->_getURL($destinationPage) .'" class="btn" >'. $text .'</a>'; // then make $text a link
							}
					}
					else{
								$text = '<a href="'. $this->_getURL($destinationPage) .'" class="btn">'. $text .'</a>'; // then make $text a link	
		...
		if($style){
			if($style=="btn")
			{
				$style = 'class="current_selected_page" ';
			}else
			{
								$style = 'class="'. $style .'"';
		...
		$temp_tile='title="Last page ('.$this->getTotalPages().")\"";
		if($onClick=="" && $style==""  && ($title=='title="First page"') && $text=="First"  ){
//			$style="class='btn' style='margin-left:5px;background-image: -moz-linear-gradient(center top , #C9C9C9, #C9C9C9);'";	
			$style="class='current_selected_page' style='font-weight:normal;'";	
		}
		elseif($onClick=="" && $style==""  && ($title=='title="Previous page"') && $text=="Previous"   ){
//			$style="class='btn' style='margin-left:5px;background-image: -moz-linear-gradient(center top , #C9C9C9, #C9C9C9);'";	
			$style="class='current_selected_page' style='font-weight:normal;'";	
		}elseif($onClick=="" && $style==""  && ($title=='title="Next page"') && $text=="Next"  ){
			//$style="class='btn' style='margin-left:5px;background-image: -moz-linear-gradient(center top , #C9C9C9, #C9C9C9);'";	
			$style="class='current_selected_page' style='font-weight:normal;'";	
		}
		elseif($onClick=="" && $style==""  && ($title==$temp_tile) && $text=="Last"   ){
			$style="class='current_selected_page' style='font-weight:normal;'";	
		}
		else{
	...
	*******************************************************/
	...
	{	
		...
		$output  = $this->_navBox("<i class='fa fa-angle-double-left'></i>", $firstPage, 1);	// First page
		$output .= $this->_navBox("<i class='fa fa-angle-left'></i>", $previousPage, 2);		// Previous page
		
		for($i = $sectionStart; $i <= $sectionEnd; ++$i)
			$output .= $this->_navBox($i, $i);					// Pagination
		
		$output .= $this->_navBox("<i class='fa fa-angle-right'></i>", $nextPage, 3);			// Next Page
		$output .= $this->_navBox("<i class='fa fa-angle-double-right'></i>", $lastPage, 4);		// Last Page
